...
String linkCurrentWeather = "http://api.openweathermap.org/data/2.5/weather?lat=45.96&lon=12.60&appid=[your key]&lang=it";
String linkForecast = "http://api.openweathermap.org/data/2.5/forecast?q=[your city name]&appid=[your key]";
...
void mostraPrevisioni(){
  ...
  display.print("Alba: " + String(stamp.hour) + ":" + String(stamp.minute));
  display.setCursor(5, 43);
  stamp.getDateTime(sunset);
  display.print("Tramonto: " + String(stamp.hour) + ":" + String(stamp.minute));
  display.display();
  delay(2000);
  display.clearDisplay();
  display.display();
  ...
  for(int i = 1; i < 9; i = i + 2){
    ...
    //ATTENTION, what i have done here, could not work for other countries.
    if((forecast_dt[i]).toInt() - (3600 * yourGMT) > tempsunrise && (forecast_dt[i]).toInt() - (3600 * yourGMT) < tempsunset){
      giorno = true;
    ...
    if((forecast_dt[i+1]).toInt() - (3600 * yourGMT) > tempsunrise && (forecast_dt[i+1]).toInt() - (3600 * yourGMT) < tempsunset){
      giorno = true;
